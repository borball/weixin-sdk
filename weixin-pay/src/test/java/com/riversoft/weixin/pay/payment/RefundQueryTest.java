...
public class RefundQueryTest {
    ...
    public void testXml2Bean() throws IOException {
        RefundQuery refundQuery = XmlObjectMapper.defaultMapper().fromXml(getXml(), RefundQuery.class);
        Assert.assertNotNull(refundQuery);
        Assert.assertNotNull(refundQuery.getRefunds());
        Assert.assertEquals(2, refundQuery.getRefunds().size());
